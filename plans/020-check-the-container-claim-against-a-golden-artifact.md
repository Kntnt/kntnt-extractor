...
> **One change is expected rather than drift** (2026-08-21): `tests/Integration/sealed-writer-test.php` grew #35's non-UTF-8 segment-name assertions (`121e8ac`), which this plan scopes out and defers to #35 by name in three places, and one comment sentence about the default chunk bounds (`d87ea1f`, ADR-0023). Re-baseline against the branch as it stands, confirm those are the only changes to that file, and proceed. Nothing in this plan's design depends on that file's contents.

## Corrections applied before execution

Six statements in this plan were amended in place on **2026-08-21**, at the start of executing it, from the independent verification recorded on **#44**; two more were added to steps 1 and 4 during execution, from what the runs actually did. Each amendment is dated where it sits, so the plan reads correctly to whoever picks it up next rather than only to whoever reads the ticket thread. None of them changes the three decisions this plan settles — `v0.5.1` as the cut, a test-only reader, and never regenerating the fixture — and none changes the design: the drift check now names #35's landing as the expected change rather than as a STOP condition, the 0.5.1 → 0.6.0 rewrite is six commits and not five, Leg 3 supports an audit rather than preventing a substitution, the drift control's refusal is recorded as the deterministic past-the-end one rather than the coin-flip `PHP_INT_MAX` one, step 2's `require_once` is guarded so the red step fails readably instead of fatally, and step 1's `X.Y.Z` instruction is dropped as a leftover that ADR-0024 had already overtaken. The two added during execution are both about what a run does rather than what the plan intends: step 4's control has to be driven against `golden-container-test.php` alone, because the drift kills an earlier test file at exit 255 before the suite reaches it, and step 2's expectations must quote §3's constant table rather than the runtime's `SODIUM_*` constants, because the harness's `sodium_compat` puts `SODIUM_CRYPTO_BOX_SEALBYTES` at 16 where the format and libsodium put it at 48.
...
**Because it is the newest tag whose writer is not the writer under test.** `git diff v0.6.0..HEAD -- classes/Crypto/Sealed_Writer.php` is empty: the shipped 0.6.0 writer and today's are the same file. A fixture cut from 0.6.0 would therefore be produced by the code it is meant to check — a tautology on the day it lands, and the ticket's own warning ("a golden file that is regenerated whenever it fails proves nothing") applied one step earlier. The 0.5.1 writer is a genuinely different implementation: 0.5.1 → 0.6.0 rewrote it by 304 insertions and 109 deletions across six commits — `f5a1e0a`, `c39e871`, `0842a8a`, `a2d7c01`, `e08ea3d`, `0b90bde`, as `git log --oneline v0.5.1..v0.6.0 -- classes/Crypto/Sealed_Writer.php` reproduces (corrected 2026-08-21: this said five) — moving the index from an in-memory list to a sidecar file (`f5a1e0a`) and `add_segment()` from a stream parameter to a string (`a2d7c01`). `f5a1e0a` is the commit ADR-0014 records, so the claim and the rewrite it is a claim about are the same change. That the two still frame identically is the whole content of the claim.
...
**Leg 3 — provenance is pinned to git, so the claimed origin can be audited.** `tests/Fixtures/README.md` records the tag, the commit, and the git blob hashes of the two class files the fixture was produced by, all of which are checkable with `git rev-parse` against history that a fixture regeneration does not touch. Corrected 2026-08-21: this leg used to claim that a regeneration "cannot quietly restate" the provenance, and it cannot do that. A regeneration can simply leave the README untouched, and nothing machine-ties the committed `.b64` bytes to those hashes, because the fixture's keys and nonces are random and its output therefore cannot be re-derived and compared. The hashes attest to what this plan *says* produced the bytes, not to what actually produced them — they support an audit, they do not prevent a substitution. Leg 1 is the load-bearing one, as the sentence two paragraphs above already says.
...
2. Write `tests/Fixtures/container-0.5.1-recipe.php` **verbatim** — the byte counts in this plan are this recipe's, and a changed literal changes them. (Corrected 2026-08-21: this used to say "except that each `X.Y.Z` placeholder becomes the version from `kntnt-extractor.php`'s `Version:` header". The block below carries no such placeholder, and the plan's own "Conventions to match" forbids stamping one under ADR-0024. Verbatim is the whole instruction.) Those counts come from the returned segments alone, so the docblocks are not among the literals they depend on:
...
**Guard the `require_once` as well** (added 2026-08-21). That pattern works in `sealed-writer-test.php` only because `Sealed_Writer` is autoloaded. A helper under `tests/Support/` is not, so a bare `require_once` of a file that does not exist yet fatals the whole Playground run — exit 255, no TAP output at all, the failure shape plan 019 recorded for its rejected design — instead of emitting one `not ok`. Wrap the require in `file_exists()` so the red step produces a readable failure, which is the entire point of running it.
...
- **It bounds-checks before every read.** §7 requires a reader to refuse any length field that would read past the end. Refuse *before* calling `unpack()` or `substr()`, not after: measured on the prototype, a reader that unpacks first still reaches the right verdict but gets there through two PHP warnings (`unpack(): Type P: not enough input values`, then `Trying to access array offset on false`). A refusal that depends on PHP's error handling to be correct is not a refusal, and the warnings are emitted onto the same stdout the suite writes its TAP lines to. Reject a negative value too — a u64 above `PHP_INT_MAX` arrives as a negative int, and a 4-byte length field read as eight can produce one (step 4's control). Corrected 2026-08-21: the sign is not deterministic. Those eight bytes are the four length bytes plus the first four bytes of a randomly generated sealed key, so the value is negative only when that key's fourth byte is ≥ `0x80` — measured over 20,000 trials on PHP 8.5, 50.1% negative. The rejection is still required and still correct; what must not be recorded as measured is the *reason* the control is refused. Refuse the negative and the past-the-end case under one message, so the control's refusal is the deterministic past-the-end one either way.
...
**Run the control against this file alone, not through the whole suite** (added 2026-08-21, measured). A full `composer test:integration` under the drift dies at **exit 255 inside `file-packaging-test.php`** — one of the seven inline `$parse` closures walks off the end of a 2634-byte container and takes the process with it — and `file-packaging-test.php` runs *before* `golden-container-test.php` in the bootstrap's `glob()` order, so the suite never reaches the assertions the control is about. That is the same failure shape plan 019 recorded, arriving from a different direction, and it is not a defect this plan may fix (those closures are out of scope). Drive `golden-container-test.php` on its own instead — a one-step blueprint mounting the plugin, a stub `kntnt_extractor_assert()`, and `require` of the three files it needs — which reproduces the split exactly: AC3 red, AC1 and AC2 green, no PHP diagnostic. Record the full-suite run too; "the drift is caught before the golden test even runs" is worth knowing.
...
| The `pack( 'V' )` drift control | 2634 bytes; the reader refuses it outright — **past the end of the container body**, which is deterministic — so the framing comparison cannot pass. Corrected 2026-08-21: the prototype recorded a `PHP_INT_MAX` message here, which arrives on roughly half of all runs and not on the other half (see the reader's bounds-checking rule) |
