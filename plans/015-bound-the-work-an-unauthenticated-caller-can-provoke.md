...
`docs/adr/0003-generic-manifest-and-table-list-no-server-side-categorisation.md`, Consequences: "A resource name that doesn't already exist (an unknown table, a path outside the root) is rejected before any capability check runs, and the 404 names every offender." `plans/README.md:127` records this as settled and explicitly carves out this plan's subject as the unresolved half — quoted here as that register stood when this plan was written: "no `args` schema, no element cap, no body-size cap … That half is real and was not selected for planning." **This plan implements exactly that half and nothing else.** That entry has since been rewritten in place, because the finding it names is now **bounded by #26** — this plan, ADR-0020 — and the register says so in the past tense. The quotation above is deliberately left as it was: it is the text this plan was written against, and a plan records what its author read, not what the source later became. The two new caps in this plan run *ahead of* every check ADR-0003 orders, not interleaved with them and not instead of them — a request that clears both caps is validated in precisely the sequence ADR-0003 fixed, unchanged.
