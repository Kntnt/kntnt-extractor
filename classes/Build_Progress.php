 ...
 * and the byte offset within it already sealed, the committed byte length of the
 * in-progress container, the ordered names of every segment written so far (the sealed
 * index the container is finalized with), and the identity of the file currently being
 * split into parts.
 *
...
final readonly class Build_Progress {
	 ...
	 * @since 0.1.0
	 *
	 * @return array{tables_done: int, structure_done: int, file_index: int, file_offset: int, container_bytes: int, segment_names: array<int, string>, file_size: int|null, file_mtime: int|null, table_offset: int, table_cursor: array<int, string>|null}
	 */
	...
	public static function from_array( mixed $data ): ?self {
		...
		// A progress record is a map of the build counters plus a list of segment names
		...
		return new self( $tables_done, $structure_done, $file_index, $file_offset, $container_bytes, $segment_names, $file_size, $file_mtime, $table_offset, $table_cursor );
