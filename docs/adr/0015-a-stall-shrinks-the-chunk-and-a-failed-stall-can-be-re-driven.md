...
- `failed` remains a terminal state for every purpose that counts one — it frees the concurrency slot, and the TTL sweep passes over it — yet is no longer strictly one-way. That is the narrow exception this decision buys, and it is confined to a record no release still in service writes.
