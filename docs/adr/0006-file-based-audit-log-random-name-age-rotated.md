...
The debug question that absence leaves open — "what was attempted on this job" — is answered by a separate, bounded last-N on the job record, projected onto `GET /extractions/{id}` as `attempts?`. That surface is [0016](./0016-a-job-carries-a-bounded-attempt-log-distinct-from-the-audit-log.md). It is not an audit record and it never appears here.
