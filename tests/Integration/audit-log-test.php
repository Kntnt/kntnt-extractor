...
kntnt_extractor_assert( ( $entry['api_version'] ?? null ) === 7, 'The entry records the api_version (AC1)' );
...
// Plant a spent attempt counter at the floor — the only stall that still fails the
// job, now that a shrinkable budget adapts instead of dying. A table slice reaches
// its floor only when BOTH of its bounds are there: the byte budget caps what is
// rendered, the row budget caps what is fetched, and either one still able to shrink
// is another real attempt worth making.
